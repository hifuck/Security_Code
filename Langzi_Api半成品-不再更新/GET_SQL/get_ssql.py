...
def scan_level_1(url):
    comm = os_run + 'sqlmap.py -u %s --batch --random-agent' % url
    print 'Level 1 : ' + comm
    try:
...
def scan_level_2(url):
    ...
    comm_cookie = os_run + 'sqlmap.py -u {} --cookie {} --level 2 --batch --random-agent'.format(urls,datas)
    print 'Level 2 : ' + comm_cookie
    comm_post = os_run + 'sqlmap.py -u {} --data {} --level 2 --batch --random-agent '.format(urls,datas)
...
def scan_level_3(url):
    comm = os_run + 'sqlmap.py -u %s --batch --tamper space2comment.py --random-agent' % url
    print 'Level 3 : ' + comm
    try:
        res = subprocess.Popen(comm, shell=True, stdout=subprocess.PIPE)
        result = res.stdout.read().replace('\n','')
...
def scan_level_4(url):
    urls,datas = url.split('?')[0],url.split('?')[1]
    comm_cookie = os_run + 'sqlmap.py -u {} --cookie {} --level 2 --tamper space2comment.py --batch --random-agent '.format(urls,datas)
    print 'Level 4 : ' + comm_cookie

    comm_post = os_run + 'sqlmap.py -u {} --data {} --level 2 --tamper space2comment.py --batch --random-agent '.format(urls,datas)
...
def scan_level_5(url):
    # 获取完整注入url即可
    comm = os_run + 'sqlmap.py -u %s --batch --tamper space2comment.py --delay 2 --time-sec=15 --timeout=20  --level 5 --random-agent' % url
    print 'Level 5 : ' + comm
    try:
        res = subprocess.Popen(comm, shell=True, stdout=subprocess.PIPE)
        result = res.stdout.read().replace('\n','')
...
def scan_html(url):
    urls = url.replace('.shtml','*.shtml').replace('.html','*.html').replace('.htm','*.htm').replace('.shtm','*.shtm')
    comm = os_run + 'sqlmap.py -u %s --batch --random-agent' % urls
    print 'Level 1 : ' + comm
    try:
        res = subprocess.Popen(comm, shell=True, stdout=subprocess.PIPE)
        result = res.stdout.read().replace('\n','')
