...
def scan(ip, username, password, port):

    print 'Scan:' + ip + ':' + username + '|' + password + ':' + str(port)
    try:
        connx = pymysql.connect(host=ip, user=username, passwd=str(password), db='mysql', port=port)
        cur = connx.cursor()
        sql = 'show databases;'
        cur.execute(sql)
        res = cur.fetchall()
...
if __name__ == '__main__':
    ...
    for ip in IP_list:
        ...
        if ress == '666':
            ...
            for password in Password_list_:
                p.apply_async(scan, args=(ip,user,password,port))
            p.close()
            p.join()
